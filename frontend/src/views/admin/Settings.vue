<template>
  <div class="page-container">
    ...
    <div class="space-y-6"><BaseCard title="Platform Configuration"><form @submit.prevent="saveSettings" class="space-y-4"><div class="grid grid-cols-1 md:grid-cols-2 gap-4"><BaseInput v-model="settings.platform_name" label="Platform Name" required /><BaseInput v-model="settings.support_email" label="Support Email" type="email" required /><BaseInput v-model="settings.support_phone" label="Support Phone" required /><BaseInput v-model.number="settings.commission_rate" label="Commission Rate (%)" type="number" step="0.01" required /></div><BaseButton type="submit" :loading="saving">Save Platform Settings</BaseButton></form></BaseCard>
      <BaseCard title="Payment Gateway"><form @submit.prevent="saveSettings" class="space-y-4"><div class="grid grid-cols-1 md:grid-cols-2 gap-4"><BaseInput v-model="settings.paystack_public_key" label="Paystack Public Key" required /><div class="flex items-center"><p class="text-sm text-gray-500 italic mt-6">Secret key is configured server-side via environment variable (PAYSTACK_SECRET_KEY)</p></div></div><div class="flex items-center gap-2"><input type="checkbox" v-model="settings.enable_test_mode" class="checkbox" id="testMode"><label for="testMode" class="text-sm font-medium text-gray-700">Enable Test Mode</label></div><BaseButton type="submit" :loading="saving">Save Payment Settings</BaseButton></form></BaseCard>
