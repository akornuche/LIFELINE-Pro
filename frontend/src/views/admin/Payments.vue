<template>
  <div class="page-container">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
      ...
      <div class="stat-card">
        <ClockIcon class="h-8 w-8 text-orange-600 mb-2" />
    ...
    <BaseCard class="mb-6">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <BaseInput v-model="filters.search" placeholder="Search reference..." @input="handleSearch" />
        <select v-model="filters.type" class="input" @change="loadPayments">
    ...
    <BaseCard>
      <BaseTable :columns="columns" :data="payments" :loading="loading" emptyText="No payments found">
        ...
        <template #cell-payer="{ row }">
          <p class="font-medium">{{ row.payer_name }}</p>
          <p class="text-xs text-gray-500">{{ row.payer_email }}</p>
        </template>
        <template #cell-type="{ value }">
          <span class="badge badge-info capitalize">{{ (value || '').replace(/_/g, ' ') }}</span>
        </template>
        <template #cell-amount="{ value }">
          <span class="font-semibold text-gray-900">₦{{ formatMoney(value) }}</span>
        </template>
        ...
        <template #actions="{ row }">
          <button @click="viewPaymentDetails(row.id)" class="text-blue-600 hover:text-blue-700 text-sm font-medium">View</button>
        </template>
      </BaseTable>
...
const loadPayments = async () => {
  loading.value = true;
  try {
    const data = await adminStore.getPayments({
      ...filters.value,
...
const viewPaymentDetails = async (id) => {
  try {
    ...
    if (data) {
      const details = [
        `Reference: ${data.reference || data.payment_reference || 'N/A'}`,
        `Amount: ₦${formatMoney(data.amount)}`,
        `Status: ${data.status}`,
        `Type: ${(data.payment_type || 'N/A').replace(/_/g, ' ')}`,
...
const getStatusBadge = (s) => ({ completed: 'badge badge-success', pending: 'badge badge-warning', failed: 'badge badge-error' }[s] || 'badge');
</script>
